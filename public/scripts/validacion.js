...
function validarFormulario(evento) {
    ...
    let elementoActivo = document.querySelectorAll('input[type="radio"]:checked')
    if (elementoActivo) {
        for (let index = 0; index < elementoActivo.length; index++) {
            document.querySelectorAll('.form-box').forEach((box) => {
            const boxInput = box.querySelector('input');
            const boxSelect = box.querySelector('select');
                if (elementoActivo[index].name == boxInput?.name) {
                    if (elementoActivo[index].value == 'si') {
                        if (boxSelect?.name === `puntaje${boxInput?.name}`) {
                            if (boxSelect.value == 0 || boxSelect.value == undefined || boxSelect.value == null || boxSelect.value == '' ) {
                                select.push(boxInput?.name);
                                contSelect++;
                            }  
                        } 
                    } 
    ...
    if (contSelect > 0) {
        swal({
            title: "Mensaje de error",
            text: "No has seleccionado cuanto te gusta, en la siguientes preguntas: "+ select,
            icon: "error",
            button: "Aceptar",
        });
        return;
    ...
    const div = document.querySelector(".spinners-envio");
